...
<div class="login-main">
    ...
    <!-- 表单选项 -->
    <form class="layui-form" action="<%=basePath%>reader/addReader" method="post">
        <div class="layui-input-inline">
            <div class="layui-inline" style="width: 85%">
                <input type="text" id="user" name="readerId" required  lay-verify="required" placeholder="请输入学号" autocomplete="off" class="layui-input">
            </div>
        ...
        <hr style="width: 85%" />
        <p style="width: 85%"><a href="<%=basePath%>system/login" class="fl">已有账号？立即登录</a></p>
    </form>
</div>
...
<script type="text/javascript">
    layui.use(['form','jquery','layer'], function () {
        ...
        $('#user').blur(function() {
            ...
            //alert(user);
            $.ajax({
                url:'<%=basePath%>reader/checkReader',
                type:'post',
                dataType:'json',
                data:{readerId:user},
                //验证用户名是否可用
        ...
        form.on('submit(sub)', function(data) {
            ...
            $.ajax({
                ...
                success:function(data){
                    if (data.success) {
                        layer.msg(data.message);
                        location.href = "<%=basePath%>system/login";
                    }else {
                        layer.msg(data.message);
                    }
                }
